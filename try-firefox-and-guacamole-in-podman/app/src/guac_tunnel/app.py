...
async def tunnel(websocket: WebSocket) -> None:
    ...
    try:
        ...
        # answers `ready` before it has connected to the VNC server, so a dead
        # Xvnc surfaces as an `error` instruction *inside* the session stream.
        # The browser reports that; there is nothing to catch here.

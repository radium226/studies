...
async def test_command_abortion(dbus_server):
    ...
    run_introspection = await bus.introspect("com.radium226.CommandExecutor", run_path)
...
async def test_attach_to_aborted_run(dbus_server):
    ...
    for line in output_lines:
        ...
        if output_section_started and line.strip():
            assert (
                "after abort" not in line
            )  # Should not be present in actual output since command was aborted
...
async def test_manual_cleanup_trigger(dbus_server):
    ...
    # Get initial stats
    await executor_interface.call_get_cleanup_stats()
...
async def test_client_cleanup_commands_integration(dbus_server):
    ...
    if runs:
        ...
        assert (
            f"Removed run {execution_id}" in remove_output
            or "not found or cannot be removed" in remove_output
        )
...
@pytest.mark.asyncio
async def test_environment_variable_propagation(dbus_server):
    """Test that environment variables are properly propagated to commands"""
...
@pytest.mark.asyncio
async def test_working_directory_propagation(dbus_server):
    """Test that working directory is properly propagated to commands"""
    import os
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            # Execute a command that checks the current directory
...
@pytest.mark.asyncio
async def test_environment_and_directory_combined(dbus_server):
    """Test that both environment variables and working directory work together"""
    import os
    import tempfile
    
    # Set a custom environment variable
    original_env = os.environ.get("TEST_COMBINED_VAR")
    os.environ["TEST_COMBINED_VAR"] = "combined_test_value"
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                # Execute a command that uses both environment and working directory
                command = ["sh", "-c", "echo $TEST_COMBINED_VAR > output.txt && pwd && cat output.txt"]
                ...
                output = captured_output.getvalue()
                assert exit_code == 0
                assert "combined_test_value" in output
        ...
        # Restore original environment
        if original_env is not None:
            os.environ["TEST_COMBINED_VAR"] = original_env
        else:
            del os.environ["TEST_COMBINED_VAR"]


@pytest.mark.asyncio
async def test_run_info_includes_environment_and_directory(dbus_server):
    """Test that run info includes environment and working directory information"""
    import os
    import tempfile
    
    # Set a custom environment variable
    original_env = os.environ.get("TEST_INFO_VAR")
    os.environ["TEST_INFO_VAR"] = "info_test_value"
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                # Get the run path
                run_path = await executor_interface.call_get_run_path(last_run_id)
                ...
                assert "working_dir" in run_info
                assert run_info["working_dir"].value == temp_dir
                
                # Check that environment count is included
                assert "env_count" in run_info
                assert run_info["env_count"].value > 0  # Should have environment variables
        ...
        # Restore original environment
        if original_env is not None:
            os.environ["TEST_INFO_VAR"] = original_env
        else:
            del os.environ["TEST_INFO_VAR"]

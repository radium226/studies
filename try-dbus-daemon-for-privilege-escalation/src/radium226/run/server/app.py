...
class RunCleanupService:
    ...
    def start_automatic_cleanup(self, runs_dict: dict[str, Any], bus: MessageBus) -> None:
        ...
        if self._cleanup_task is None or self._cleanup_task.done():
            ...
            logger.info(
                f"Started automatic cleanup task (interval: {self.config.cleanup_interval_minutes}m)"
            )
    ...
    async def _automatic_cleanup_loop(self, runs_dict: dict[str, Any], bus: MessageBus) -> None:
    ...
    def cleanup_runs(self, runs_dict: dict[str, Any], bus: MessageBus) -> int:
        """
        ...
        sorted_runs = sorted(runs_dict.items(), key=lambda x: x[1].start_time)
        ...
        if len(completed_runs) > self.config.max_completed_runs:
            ...
            for i in range(excess_completed):
                ...
                if execution_id not in runs_to_remove:
                    runs_to_remove.append(execution_id)
                    logger.debug(
                        f"Marking run {execution_id} for completed count cleanup"
                    )
        ...
        if len(sorted_runs) > self.config.max_total_runs:
            ...
            for i in range(excess_total):
                ...
                if execution_id not in runs_to_remove:
                    ...
                    if (
                        not self.config.keep_running
                        or sorted_runs[i][1].status != "running"
                    ):
                        runs_to_remove.append(execution_id)
                        logger.debug(
                            f"Marking run {execution_id} for total count cleanup"
                        )
        ...
        for execution_id in runs_to_remove:
            if execution_id in runs_dict:
                ...
                logger.debug(
                    f"Cleaned up run {execution_id} (status: {run.status}, age: {now - run.start_time})"
                )
...
class RunInterface(ServiceInterface):
    ...
    def GetInfo(self) -> "a{sv}":  # type: ignore  # noqa: F722
        ...
        return {
            ...
            "exit_code": Variant(
                "i", self.exit_code if self.exit_code is not None else -1
            ),
            "working_dir": Variant("s", self.working_dir or ""),
            "env_count": Variant("i", len(self.env_vars)),
        }
    ...
    async def execute_async(self) -> None:
        ...
        try:
            logger.info(
                f"Executing command {self.execution_id}: {self.command} as user: {self.target_user}"
            )
            ...
            if current_uid == 0:  # Running as root
                try:
                    ...
                    logger.info(
                        f"Server running as root, switching to user {self.target_user} (uid: {target_uid}, gid: {target_gid})"
                    )
                except KeyError:
                ...
                if current_user != self.target_user:
                    raise Exception(
                        f"Cannot switch from user '{current_user}' to '{self.target_user}' - server not running as root"
                    )
                logger.info(
                    f"Server running as {current_user}, no user switching needed"
                )
            ...
            if self.aborted:
                ...
                try:
                    ...
                    logger.warning(
                        f"Process {self.execution_id} did not exit after terminate, killing it"
                    )
                    self.process.kill()
            ...
            if self.aborted:
                ...
                logger.info(
                    f"Command {self.execution_id} completed with exit code: {exit_code}"
                )
...
class CommandExecutorInterface(ServiceInterface):
    def __init__(self, bus: MessageBus, cleanup_config: CleanupConfig | None = None):
    ...
    @method()
    def ExecuteCommand(self, command: "as", target_user: "s", env_vars: "a{ss}", working_dir: "s") -> "s":  # type: ignore  # noqa: F722,F821
        ...
        logger.info(
            f"Starting command execution {execution_id}: {command} as user: {target_user}, cwd: {working_dir}"
        )

        # Create Run instance with sequence number, target user, environment, and working directory
        run_instance = RunInterface(
            execution_id, command, self.sequence_counter, target_user, dict(env_vars), working_dir or None
        )
    ...
    def GetCleanupStats(self) -> "a{sv}":  # type: ignore  # noqa: F722
        ...
        return {
            ...
            "cleanup_interval_minutes": Variant(
                "i", self.cleanup_config.cleanup_interval_minutes
            ),
...
async def main() -> None:
    ...
    logger.info(
        f"D-Bus server ready and listening for commands on {bus_type.name.lower()} bus"
    )

...
class TrackedFace[FaceEmbeddingT]:
    ...
    Implements the `Interpolable` protocol: `to_vector`/`from_vector` flatten and
    ...
    def to_vector(self) -> list[float]:
        ...
        return [
            ...
            bounding_box.width,
            bounding_box.height,
            *chain.from_iterable(self.face.detection.landmarks),
        ]

    def from_vector(self, vector: list[float]) -> "TrackedFace[FaceEmbeddingT]":
        x, y, width, height, *flat_landmarks = vector
        points = list(zip(flat_landmarks[0::2], flat_landmarks[1::2], strict=True))
        detection = Detection(
            bounding_box=BoundingBox(x=x, y=y, width=width, height=height),
            landmarks=(points[0], points[1], points[2], points[3], points[4]),

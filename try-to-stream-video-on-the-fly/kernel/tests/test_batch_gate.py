from video_analyzer.kernel import BatchGate, BatchGateConfig, Clock
...
def test_fires_when_batch_full() -> None:
    clock = ManualClock()
    gate = BatchGate(
        clock, 30.0, config=BatchGateConfig(max_frames=4, max_lag_ms=1000.0)
    )
...
def test_fires_when_lag_exceeded() -> None:
    clock = ManualClock()
    gate = BatchGate(
        clock, 30.0, config=BatchGateConfig(max_frames=100, max_lag_ms=50.0)
    )
    assert not gate.should_fire(1)
    clock.time += 0.1
...
def test_resets_after_firing() -> None:
    clock = ManualClock()
    gate = BatchGate(
        clock, 30.0, config=BatchGateConfig(max_frames=1, max_lag_ms=0.0)
    )
    assert gate.should_fire(1)
    assert gate.should_fire(1)
...
def test_no_fire_without_available_frames() -> None:
    clock = ManualClock()
    gate = BatchGate(
        clock, 30.0, config=BatchGateConfig(max_frames=4, max_lag_ms=0.0)
    )
...
def test_token_bucket_gates_firing() -> None:
    clock = ManualClock()
    gate = BatchGate(
        clock, 1.0, config=BatchGateConfig(max_frames=1, max_lag_ms=0.0)
    )
    assert gate.should_fire(1)

...
from video_analyzer.core.config import SplineInterpolatorConfig
...
async def test_linear_query_interpolates_midpoint() -> None:
    interpolator = SplineInterpolator(config=SplineInterpolatorConfig(method="linear"))
...
async def test_pchip_query_between_more_than_two_points() -> None:
    interpolator = SplineInterpolator(config=SplineInterpolatorConfig(method="pchip"))

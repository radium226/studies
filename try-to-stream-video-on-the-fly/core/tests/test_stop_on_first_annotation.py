...
def _annotated_frame(index: int, faces: list[int]) -> kernel.AnnotatedFrame[int, int]:
...
async def test_no_stop_when_no_faces() -> None:
    wrapped = _RecordingFrameBroadcaster()
    stop_token = kernel.StopToken()
    broadcaster = core.StopOnFirstAnnotation(wrapped, stop_token)
...
async def test_stop_requested_on_first_frame_with_a_face() -> None:
    wrapped = _RecordingFrameBroadcaster()
    stop_token = kernel.StopToken()
    broadcaster = core.StopOnFirstAnnotation(wrapped, stop_token)

...
The box-parse task (an asyncio task, since it does async subprocess I/O) is the sole producer.
Starlette request handlers (also async tasks on the same event loop) are consumers, awaiting
`wait_for_next` directly via an `asyncio.Condition` - no thread bridging needed since producer and
consumers already share the same event loop.

A bounded deque gives an automatic, allocation-free drop-oldest retention policy: total memory is
capped regardless of subscriber count or speed.
"""
...
class Broadcaster:
    ...
    def snapshot_for_new_client(self) -> Snapshot:
        ...
        No locking needed: asyncio is single-threaded/cooperative and this method never awaits,
        so it can't interleave with a concurrent publish_fragment.
        """
    ...
    async def wait_for_next(self, last_seq: int, timeout: float) -> Fragment | None:
        ...
        Returns None on timeout/no-new-data/closed. Raises LaggedError if the caller's last_seq
        has fallen behind the oldest retained fragment - callers should treat that as "can't catch
        up, end this connection."
        """
        async with self._condition:
            try:

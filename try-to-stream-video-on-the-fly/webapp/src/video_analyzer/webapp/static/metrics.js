...
// needs to redraw the latest snapshot every tick. Ported from app/'s version;
// the snapshot keys are deliberately unchanged, so this stays a direct port.
...
function render(data) {
  const rows = ROWS.map(([key, label, unit]) => {
    ...
    // Idle returns {}, so every key is missing — a dash beats a stale number.
